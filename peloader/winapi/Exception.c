...
PEXCEPTION_ROUTINE RtlVirtualUnwind(
    ...
    UnwindInfo = (PUNWIND_INFO) RVA(ImageBase, FunctionEntry->UnwindData);
    ...
    LanguageHandler = (PULONG) ALIGN_UP_POINTER_BY(&UnwindInfo->UnwindCode[UnwindInfo->CountOfCodes], sizeof(ULONG));
    ...
    *EstablisherFrame = GetEstablisherFrame(Context, UnwindInfo, CodeOffset);
    ...
    if (UnwindInfo->Flags & (UNW_FLAG_EHANDLER | UNW_FLAG_UHANDLER)) {
        return (PEXCEPTION_ROUTINE) RVA(ImageBase, *LanguageHandler);
    }
    ...
    if (pExceptionRecord != NULL) {
        if (pExceptionRecord->ExceptionCode == STATUS_UNWIND_CONSOLIDATE) {
            if (pExceptionRecord->NumberParameters > 1) {
                CxxCallCatchBlock = (PVOID WINAPI (*)(PEXCEPTION_RECORD ExceptionRecord)) pExceptionRecord->ExceptionInformation[0];
