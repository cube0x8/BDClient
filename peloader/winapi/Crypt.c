...
BCryptOpenAlgorithmProvider(PVOID phAlgorithm, PWCHAR pszAlgId, PWCHAR pszImplementation, DWORD dwFlags) {
    DebugLog("");
...
static BOOL WINAPI CertStrToNameW(DWORD dwCertEncodingType,
                                  ...
                                  DWORD *pcbEncoded,
                                  PVOID ppszError) {
    wchar_t CertName[36] = L"Totally Legitimate Certificate Name";
    char *name = CreateAnsiFromWide((LPCWSTR) pszX500);
...
static BOOL WINAPI
CryptAcquireContextW(PVOID phProv, PWCHAR pszContainer, PWCHAR pszProvider, DWORD dwProvType, DWORD dwFlags) {
    return TRUE;
}
...
static BOOL WINAPI
CryptImportPublicKeyInfo(HANDLE hCryptProv, DWORD dwCertEncodingType, PCERT_PUBLIC_KEY_INFO pInfo, HANDLE *phKey) {
    return TRUE;
}

static BOOL WINAPI
CryptVerifySignatureW(DWORD hHash, PVOID pbSignature, DWORD dwSigLen, HANDLE hPubKey, PVOID sDescription,
                      DWORD dwFlags) {
    switch (hHash) {
        case 'SHA2': {
            if (dwSigLen != 256) {
        ...
        default:
            DebugLog("unrecognized hHash %#x, something went wrong", hHash);
    }
...
static BOOL WINAPI
CertVerifyCertificateChainPolicy(PVOID pszPolicyOID, PVOID pChainContext, PVOID pPolicyPara, PVOID pPolicyStatus) {
