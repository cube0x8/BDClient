...
STATIC HANDLE WINAPI CreateFileMappingW(FILE *hFile,
...
STATIC HANDLE WINAPI CreateFileMappingA(FILE *hFile,
                                        PVOID lpFileMappingAttributes,
                                        DWORD flProtect,
                                        DWORD dwMaximumSizeHigh,
                                        DWORD dwMaximumSizeLow,
                                  ...
                                  SIZE_T dwNumberOfBytesToMap)
{
    ...
    SIZE_T file_view_size = dwNumberOfBytesToMap;
    PVOID FileView;

    MappedFileEntry *MappedFile = (MappedFileEntry *) hFileMappingObject;

    if (dwNumberOfBytesToMap == 0) {
        file_view_size = MappedFile->size - file_offset.value;
        FileView = calloc(1, file_view_size);
    }
    else {
        FileView = calloc(1, file_view_size);
    }
    ...
    memcpy(FileView, (char *) MappedFile->start + file_offset.value, file_view_size);
