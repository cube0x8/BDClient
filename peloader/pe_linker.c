...
int import(void *image, IMAGE_IMPORT_DESCRIPTOR *dirent, char *dll)
{
        ...
        for (i = 0; lookup_tbl[i]; i++) {
                if (IMAGE_SNAP_BY_ORDINAL(lookup_tbl[i])) {
                        ...
                        address_tbl[i] = (ULONG_PTR) ordinal_import_stub;
                        continue;
                }
                else {
                ...
                if (get_export(symname, &adr) < 0) {
                        ...
                        address_tbl[i] = (ULONG_PTR) unknown_symbol_stub;
                        continue;
                } else {
...
int read_exports(struct pe_image *pe)
{
        ...
        name_table = (unsigned int *)((uintptr_t) pe->image +
                                      export_dir_table->AddressOfNames);
        ordinal_table = (uint16_t *)((uintptr_t) pe->image +
                                      export_dir_table->AddressOfNameOrdinals);

        pe_exports = (struct pe_exports *) calloc(export_dir_table->NumberOfNames, sizeof(struct pe_exports));

        for (i = 0; i < export_dir_table->NumberOfNames; i++) {
                uint32_t address = ((uint32_t *) ((uintptr_t) pe->image + export_dir_table->AddressOfFunctions))[*ordinal_table];
                ...
                pe_exports[num_pe_exports].name = (char *) (((uintptr_t) pe->image) + *name_table);
                pe_exports[num_pe_exports].addr = (void (*)()) (((uintptr_t) pe->image) + address);
...
static int fixup_reloc(void *image, IMAGE_NT_HEADERS *nt_hdr)
{
        ...
        while (fixup_block->SizeOfBlock) {
                ...
                fixup_block = (IMAGE_BASE_RELOCATION *)
                        ((uintptr_t)fixup_block + fixup_block->SizeOfBlock);
        };
...
static int fix_pe_image(struct pe_image *pe)
{
        ...
        for (i = 0; i < sections; i++) {
                ...
                memcpy((void *)(((char *) image) + sect_hdr->VirtualAddress),
                       (void *)(((char *) pe->image) + sect_hdr->PointerToRawData),
...
int link_pe_images(struct pe_image *pe_image, unsigned short n)
{
        ...
        for (i = 0; i < n; i++) {
                ...
                pe->entry = (BOOL WINAPI (*)(PVOID hinstDLL, DWORD fdwReason, PVOID lpvReserved))
                        RVA2VA(pe->image,
...
{
    SharedUserData = (PKUSER_SHARED_DATA) 
                          mmap((PVOID)(MM_SHARED_USER_DATA_VA),
